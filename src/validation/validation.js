...
export const validatePhone = (phone) => {
  ...
  if (phone && !phoneRegex.test(phone)) {
    throw new AppError(ErrorCodes.INVALID_PHONE);
  }
};


/**
 * Validate profile data (fields + rules)
 */
export const validateProfileData = (profileData) => {
  ...
  if (profileData.display_name && profileData.display_name.length < 2) {
    throw new AppError(
      ErrorCodes.PROFILE_INVALID_DATA,
      { reason: "display_name_length" }
    );
  }

  // Status whitelist
  if (profileData.status &&
      !['unpublished', 'published'].includes(profileData.status)) {
    throw new AppError(
      ErrorCodes.PROFILE_INVALID_DATA,
...
/**
 * Validate account update payload
 */
export const validateAccountData = (accountData) => {
  ...
  if (invalidFields.length > 0) {
    throw new AppError(
      ErrorCodes.ACCOUNT_INVALID_DATA,
      { invalidFields }
    );
  }

  if (
    accountData.subscription_type &&
...
export const validateUUID = (uuid) => {
  ...
  if (!uuidRegex.test(uuid)) {
    throw new AppError(ErrorCodes.INVALID_UUID);
  }
};


/**
 * Validate POST /contacts body
 */
